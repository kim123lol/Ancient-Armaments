package net.kim123lol.ancientarmaments.advancement;
	...
	public void generate(final AdvancementOutput output, final Provider registries) {
		...
		final var root = AdvancementBuilder.root()
				...
				.addCriterion("unlocked_example_item_recipe",
						RecipeUnlockedTrigger.unlocked(AncientArmaments.location("example_recipe_folder/custom_recipe_name")))
				.save(output, AncientArmaments.location("example/root"));
		...
		AdvancementBuilder.child(root)
				...
				.save(output, AncientArmaments.location("example/child"));
	}
}